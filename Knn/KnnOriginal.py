...
def z_score_normalize(a):
    ...
    for row in a:
        idx = 0         # the i-th NUMERICAL attribute in input data corresponds to the i-th entry in means/std_devs
        new_tuple = ()
        for col in range(n_features):
            if col in nominal_indices:
                new_tuple += (row[col],)
            else:
                new_tuple += (z_score_formula(row[col], means[idx], std_devs[idx]),)
                idx += 1
    ...
    for i in range(len(test_input)):
        ...
        for j in range(n_features):
            ...
            if not stat:
                ...
                new_tuple += (z_score_formula(test_input[i][j], means[idx], std_devs[idx]),)
                idx += 1
        # Don't forget to reattach original clas label
...
if __name__ == '__main__':
    ...
    for i in range(folds):
        if i != folds - 1:
            ...
            test_data_idx = set(range(i * part_len, len(original_input)))

        train_data_idx = set(range(len(original_input))).difference(test_data_idx)

        train_input = []
        test_input = []

        #fill train_input and test_input
        for j in range(len(original_input)):
            if j in train_data_idx:
                train_input.append(original_input[j])
            else:
                test_input.append(original_input[j])
    ...
    print()
    print("********** Final answer ************")
    print('Average Accuracy:', metrics_avg[0])
